...
check_dependencies() {
    ...
    log "INFO" "Codex CLI must be installed and available on PATH."
...
# Main installation
main() {
    echo "🚀 Installing Ralph for Codex globally..."
    echo ""
    ...
    echo ""
    log "SUCCESS" "🎉 Ralph for Codex installed successfully!"
    echo ""
...
case "${1:-install}" in
    install)
        ...
        ;;
    uninstall)
        log "INFO" "Uninstalling Ralph for Codex..."
        ...
        rm -rf "$RALPH_HOME"
        log "SUCCESS" "Ralph for Codex uninstalled"
        ;;
    --help|-h)
        echo "Ralph for Codex Installation"
        echo ""

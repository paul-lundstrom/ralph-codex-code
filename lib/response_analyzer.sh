...
# 2. Codex CLI object format: { result, sessionId, metadata: { files_changed, has_errors, completion_status, ... } }
# 3. Codex CLI array format: [ {type: "system", ...}, {type: "assistant", ...}, {type: "result", ...} ]
parse_json_response() {
    local output_file=$1
    ...
    # Check if JSON is an array (Codex CLI array format)
    # Codex CLI outputs: [{type: "system", ...}, {type: "assistant", ...}, {type: "result", ...}]
    ...
    # Detect JSON format by checking for Codex CLI fields
    local has_result_field=$(jq -r 'has("result")' "$output_file" 2>/dev/null)

    # Extract fields - support both flat format and Codex CLI format
    # Priority: Codex CLI fields first, then flat format fields
    ...
    # Codex CLI JSON format embeds the RALPH_STATUS block within the .result text field
    ...
    # Summary: from flat format OR from result field (Codex CLI format)
    local summary=$(jq -r '.result // .summary // ""' "$output_file" 2>/dev/null)

    # Session ID: from Codex CLI format (sessionId) OR from metadata.session_id
    ...
    # Progress indicators: from Codex CLI metadata (optional)
...
analyze_response() {
    ...
    # If explicit signal found, heuristics should NOT override Codex's intent
    ...
    if grep -q -- "---RALPH_STATUS---" "$output_file"; then
        ...
        if [[ -n "$exit_sig" ]]; then
            ...
            if [[ "$exit_sig" == "true" ]]; then
                ...
                # Explicit EXIT_SIGNAL: false - Codex says to continue
                exit_signal=false
            fi
    ...
    # Codex's explicit intent takes precedence over natural language pattern matching
...
update_exit_signals() {
    ...
    # Update completion_indicators array (only when Codex explicitly signals exit)
    ...
    # This caused premature exits after 5 loops. Now we respect Codex's explicit intent.
...
SESSION_FILE="$RALPH_DIR/.codex_session_id"
